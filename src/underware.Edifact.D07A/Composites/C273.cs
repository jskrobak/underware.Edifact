using System;
using System.Collections.Generic;
using System.Text;
using underware.Edifact.Attributes;

namespace underware.Edifact.D07A.Composites
{
    public class C273
    {
				[ElementInfo(0, "Item description code", "Conditional", "", "", "" )]
public string E7009 { get; set; }
		[ElementInfo(1, "Code list identification code", "Conditional", "", "", "" )]
public string E1131 { get; set; }
		[ElementInfo(2, "Code list responsible agency code", "Conditional", "", "", "" )]
public string E3055 { get; set; }
		[ElementInfo(3, "Item description", "Conditional", "", "", "" )]
public string E7008 { get; set; }
		[ElementInfo(4, "Item description", "Conditional", "", "", "" )]
public string E7008_0 { get; set; }
		[ElementInfo(5, "Language name code", "Conditional", "", "", "" )]
public string E3453 { get; set; }

    }
}